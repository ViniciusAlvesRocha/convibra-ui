...
@Component({
	selector: 'app-manage-issuer',
	templateUrl: './manage-issuer.component.html',
	styleUrls: ['./manage-issuer.component.css']
})
export class ManageIssuerComponent implements OnInit {
	...
	ngOnInit() {
		this.getIssuerListing(this.currentPage, this.perPage, this.searchTerm);
	...
	/* getInstanceSetting(){
        ...
        this.commonService.getInstanceSetting(paramData).subscribe(success => {
            ...
            this.titleService.setTitle(this.instanceSettingObj.domainName+": Issuer List");
	...
	getIssuerListing(pageNumber, limit, searchTerm) {
		this.userService.getIssuerList(pageNumber, limit, searchTerm).subscribe(success => {
			...
			this.alertService.showError(error.message);
			this.userList = [];
			// console.log('Issuer Listing Error:',error.message);
		})
	}
	...
	pageChange(page) {
		if (!isNaN(page)) {
			this.getIssuerListing(page, this.perPage, this.searchTerm);
		}
	}
